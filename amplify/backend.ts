...
// create a new Lambda integration
const mediaPipelineLambdaIntegration = new LambdaIntegration(
  backend.createMediaPipeline.resources.lambda
);
// stop media pipeline
const stopMediaPipelineLambdaIntegration = new LambdaIntegration(
  backend.createMediaPipeline.resources.lambda
);

// create a new resource path with IAM authorization
const mediaPipelinePath = mediaPipelineRestApi.root.addResource("mediapipeline", {
  defaultMethodOptions: {
    //authorizationType: AuthorizationType.IAM,
  },
});
...
// add methods you would like to create to the resource path
//itemsPath.addMethod("GET", lambdaIntegration);
mediaPipelinePath.addMethod("POST", mediaPipelineLambdaIntegration);
mediaPipelinePath.addMethod("DELETE", stopMediaPipelineLambdaIntegration);
//itemsPath.addMethod("PUT", lambdaIntegration);
...
backend.addOutput({
  custom: {
    API: {
      ...
      [mediaPipelineRestApi.restApiName]: {
        endpoint: mediaPipelineRestApi.url,
        region: Stack.of(mediaPipelineRestApi).region,
        apiName: mediaPipelineRestApi.restApiName,
      },
    },
  },
});